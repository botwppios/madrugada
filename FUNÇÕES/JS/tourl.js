...
.then(post => {
  console.log(post); // Adicione este log para verificar o formato da resposta
  if (post[0] && post[0].src) {
    resolve('https://telegra.ph' + post[0].src);
  } else {
    reject(new Error('Upload failed or invalid response'));
  }
})
      .catch(error => reject(error));
    } catch (error) {
      console.error(error);
      reject(error); // Use reject para passar o erro ao invés de apenas logá-lo.
    }
  });
}
