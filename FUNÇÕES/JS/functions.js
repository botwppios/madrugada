...
exports.fetchJson = fetchJson = async (url, options) => {
...
return `${pad(dias)} Dia(s) ${pad(horas)}:${pad(minutos)}:${pad(segundos)}`;
}
...
function adicionarLink(link, nomeArquivo) {
...
if (conteudoAtual.includes(link)) {
console.log('O link já existe na lista.');
return;
}            
conteudoAtual.push(link);
fs.writeFile(nomeArquivo, JSON.stringify(conteudoAtual, null, 2), (err) => {
if (err) {
...
module.exports = { fetchJson, fetchText, generateMessageID, getGroupAdmins, getMembros, getRandom, temporizador, recognize, bgcolor, isFiltered, addFilter, chyt, getExtension, convertSticker, nit, getpc, supre, FormData, runtime, adicionarLink, sleep, getBuffer }
